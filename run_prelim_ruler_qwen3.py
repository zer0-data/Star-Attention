"""Single-GPU preliminary RULER (v1) sweep for Qwen3-4B-Instruct-2507.

Drop-in equivalent of ``run_prelim_ruler.py`` with defaults wired for
``Qwen/Qwen3-4B-Instruct-2507`` (or any other Qwen3 checkpoint via
``--model_path``).  The underlying logic — data generation, inference,
scoring — is identical; only the model path, prompt template, and
output-directory defaults differ.

Model properties (Qwen3-4B-Instruct-2507)
------------------------------------------
* Architecture  : qwen3 (GQA, 32 Q-heads / 8 KV-heads, 36 layers)
* Parameters    : 4.0 B total / 3.6 B non-embedding
* Context       : 262,144 tokens natively (rope_theta = 5,000,000)
* Thinking mode : disabled — outputs clean answers with no <think> blocks
* Chat format   : ChatML  (<|im_start|> / <|im_end|>)
* EOS tokens    : 151645 (<|im_end|>)  and  151643 (<|endoftext|>)

Model detection is automatic: ``model.py`` reads ``config.model_type`` at
load time and selects ``Qwen3ForCausalLM`` from ``star_attention/`` instead
of the Llama class, so no code changes are needed at the inference level.

Example
-------
  python run_prelim_ruler_qwen3.py \\
...
The model is loaded ONCE; summary-related attributes are mutated in-place
between sweep cells so the ~8 GB of Qwen3-4B weights stay resident on GPU.
"""
...
# --------------------------------------------------------------------------- #
# Helpers (identical to run_prelim_ruler.py)
# --------------------------------------------------------------------------- #
...
def main(args):
    ...
    print("=" * 72)
    print("  Statistical Cross-Attention — RULER v1 Prelim Sweep (Qwen3-4B-Instruct-2507)")
    print("=" * 72)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Single-GPU RULER v1 prelim sweep for Qwen3 models."
    )

    # Model — Qwen3-specific defaults
    parser.add_argument(
        "--model_path",
        default="Qwen/Qwen3-4B-Instruct-2507",
        help="HuggingFace model ID or local path to a Qwen3 checkpoint.",
    )
    parser.add_argument(
        "--prompt_config",
        default="qwen3",
        choices=list(PROMPT_TEMPLATES.keys()),
        help="Prompt template (default: qwen3). See ruler/data/template.py.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "CSV list of summary methods: anchor, tfidf, bm25, "
            "entropy, max_idf, evenly_spaced, mean_pool."
        ),
    )
    parser.add_argument(
        "--summary_chunks",
    ...
    parser.add_argument(
        ...
        help="CSV list of RULER v1 tasks.",
    )
    parser.add_argument(
        "--seq_lengths",
        default="16384,32768",
        help=(
            "CSV list of context sequence lengths. "
            "Qwen3-4B-Instruct-2507 supports up to 262,144 tokens natively."
        ),
    )

    # Fixed Star-Attention settings
    ...
    # Dataset — Qwen3-specific output dirs
    parser.add_argument(
        "--data_dir",
        default=os.path.join(BASE_DIR, "dataset", "prelim_ruler_qwen3_4b_instruct_2507"),
        help="Root for cached RULER data (layout: <data_dir>/<seq_length>/<task>/validation.jsonl).",
    )
    ...
    # Output — Qwen3-specific paths
    parser.add_argument(
        "--predictions_dir",
        default=os.path.join(BASE_DIR, "results", "prelim_ruler_qwen3_4b_instruct_2507", "predictions"),
        help="Where per-cell prediction jsonl files are written.",
    )
    parser.add_argument(
        "--results_file",
        default=os.path.join(BASE_DIR, "prelim_ruler_qwen3_4b_instruct_2507_accuracies.txt"),
